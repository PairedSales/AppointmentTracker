...
export function AppraisalTable({
  ...
  return (
    <section className="table-container" ref={parentRef}>
          ...
          {isLoading && filteredAppraisals.length === 0 ? (
            ...
            <>
              {rowVirtualizer.getVirtualItems().length > 0 && rowVirtualizer.getVirtualItems()[0].start > 0 && (
                <tr><td colSpan={viewMode === 'completed' ? 10 : 9} style={{ height: rowVirtualizer.getVirtualItems()[0].start, padding: 0, border: 'none' }} /></tr>
              )}
              {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                const app = filteredAppraisals[virtualRow.index];
              ...
              {rowVirtualizer.getVirtualItems().length > 0 && rowVirtualizer.getTotalSize() - rowVirtualizer.getVirtualItems()[rowVirtualizer.getVirtualItems().length - 1].end > 0 && (
                <tr><td colSpan={viewMode === 'completed' ? 10 : 9} style={{ height: rowVirtualizer.getTotalSize() - rowVirtualizer.getVirtualItems()[rowVirtualizer.getVirtualItems().length - 1].end, padding: 0, border: 'none' }} /></tr>
              )}
            </>
          )}
        </tbody>
...
export default React.memo(AppraisalTable);
